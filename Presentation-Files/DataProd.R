...
for(i in 1:7)
{
          #country1[,i] <- as.matrix(country[complete.cases(country[,i]),i])
          #country1<- as.matrix(country$`North America`[c(-pos:-length(country$North.America))])
          #pos <- which(is.na(country$`North America`))[1] - 1
